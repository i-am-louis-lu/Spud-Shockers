...
export class Game {
  ...
  // Load a MagicaVoxel .vox file as the active map. Clears the procedural
  // buildings + obstacles (ground/sky/perimeter stay), drops the merged
  // voxel mesh into the scene, adopts its AABB obstacles for collision, and
  // overrides team spawn points if the .vox provided marker voxels.
  //
  // Safe to fail: if the .vox doesn't exist or parses badly, the classic
  // arena is left intact. Caller can await this Promise to detect failure.
  async loadVoxMap(url) {
    const { loadVoxMap } = await import('./voxmap.js');
    let map;
    try {
      map = await loadVoxMap(url);
    } catch (err) {
      console.warn('[voxmap] failed to load', url, err);
      return false;
    }
    // Remove procedural building meshes + their obstacles. Decorative meshes
    // added via Arena.addDecor (trees, banners, etc.) stay — they live on
    // arena.scene only, not in obstacles. Ground / boundary / perimeter
    // are also kept since they don't carry an `o.mesh` we tracked.
    ...
    // Adopt voxel-derived obstacles for collision. Same shape as Arena's:
    // {x, y, z, w, h, d} corner-based AABB.
    for (const o of map.obstacles) this.arena.obstacles.push(o);
    // Drop the merged voxel mesh in
    this.scene.add(map.mesh);
    // Override spawns ONLY if the map provided some (palette markers 254/255)
    if (map.spawns.mash.length > 0) {
      this.arena.teamSpawns.mash = map.spawns.mash.map(
        (s) => new THREE.Vector3(s.x, s.y, s.z),
      );
    }
    if (map.spawns.russet.length > 0) {
      this.arena.teamSpawns.russet = map.spawns.russet.map(
        (s) => new THREE.Vector3(s.x, s.y, s.z),
      );
    }
    // Recompute spawn-zone AABBs so anti-spawn-camp logic works. Approximate
    // with the spawn list's bounding box, padded by 4m.
    const spawnZone = (spawns) => {
    ...
    if (map.spawns.mash.length)   this.arena.teamSpawnZones.mash   = spawnZone(map.spawns.mash);
    if (map.spawns.russet.length) this.arena.teamSpawnZones.russet = spawnZone(map.spawns.russet);
    // Rebuild the nav grid since obstacles changed. NavGrid samples obstacles
    // at construction; cached pathfinding from the old grid is now stale.
    this.navGrid = new NavGrid(this.arena, 1);
    // Save a reference so the start-screen UI can show "voxel map loaded".
    this._voxMap = map;
    return true;
  }
